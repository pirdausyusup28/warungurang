...
<main class="content">
    ...
    <div class="container-fluid p-0">
        <h1 class="h3 mb-3"><strong>Gudang</strong> Data</h1>
        <div class="row">
            <div class="">
                        ...
                        <tbody>
                          @foreach ($data as $index => $d)
                            <tr>
                              ...
                              <td>{{ $d->nama_gudang}}</td>
                              <td>{{ $d->lokasi}}</td>
                              <td>{{ $d->longitude }} , {{ $d->latitude }}</td>
                              <td>{{ $d->deskripsi }}</td>
                              <td>
                                <button type="button" class="btn btn-primary" onclick="editdata({{ $d->id_gudang }})">Edit</button>
                                <button type="button" class="btn btn-danger" onclick="confirmDelete({{ $d->id_gudang }})">Hapus</button>

                                <!-- Form untuk hapus -->
                                <form method="POST" action="{{ route('karyawan.destroy', $d->id_gudang) }}" id="formDelete{{ $d->id_gudang }}">
                                    @csrf
                                    @method('DELETE')
                                </form>
                              </td>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Form Data Gudang</h5>
          ...
          <form action="{{ route('gudang.store') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                <div class="mb-3">
                  <label for="formGroupExampleInput2" class="form-label">Nama Gudang</label>
                  <input type="text" class="form-control" id="nama_gudang" name="nama_gudang" required placeholder="">
                </div>
                <div class="mb-3">
                  <label for="formGroupExampleInput2" class="form-label">Lokasi</label>
                  <input type="text" class="form-control" id="lokasi" name="lokasi" required placeholder="">
                </div>
                <div class="mb-3">
                  <label for="formGroupExampleInput2" class="form-label">Lat</label>
                  <input type="text" class="form-control" id="latitude" name="latitude" required placeholder="">
                </div>
                <div class="mb-3">
                  <label for="formGroupExampleInput2" class="form-label">Long</label>
                  <input type="text" class="form-control" id="longitude" name="longitude" required placeholder="">
                </div>
                <div class="mb-3">
